...
public class Swea2383_점심_식사시간_re {
...
//			if (cnt == 3)
			...
			for (int i = 0; i<selected.length; i++) {
				...
				if (selected[i]) { // 계단 1로 내려감
					s1People[i1] = person.time1 + stair1 + 1;
					i1++;
				} else { // 계단 2로 내려감
					s2People[i2] = person.time2 + stair2 + 1;
					i2++;
				}
			}
...
//		System.out.println("################"+Arrays.toString(sPeople));
		while (true) {
//			System.out.println(time+"//"+onStair+"//"+stair+": ################"+Arrays.toString(sPeople));
			...
			if (waitIdx != -1) {
				for (int i = waitIdx; i < sPeople.length; i++) {
					if (sPeople[i] == stair) {
						sPeople[i]++;
						onStair--;
					}
